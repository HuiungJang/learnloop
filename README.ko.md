...
4. OAuth를 선택한 경우 local AI setup 화면에서 `Connect Codex` 또는 `Connect Gemini`를 눌러 이 컴퓨터의 provider 로그인 명령을 시작합니다.
5. 로컬 AI 설정을 저장한 뒤 학습 플로우를 실행합니다.
6. 생성된 패턴 카드와 실습 문제를 검수하고 재사용 가능한 학습 자산으로 축적합니다.
